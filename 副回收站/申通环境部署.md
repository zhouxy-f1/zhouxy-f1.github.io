...
| IP          | 主机名  | 安装软件                                                |
| ----------- | ------- | ------------------------------------------------------- |
| 10.1.20.203 | server1 | GlusterFS、K8S-master、Kafka、PostgreSQL、Pgpool、Redis |
| 10.1.20.251 | server2 | GlusterFS、K8S-node1、Kafka、 PostgreSQL、Pgpool        |
| 10.1.20.197 | server3 | GlusterFS、K8S-node2、Kafka                             |
| 10.1.20.195 | server4 | GlusterFS、K8s-node3                                    |
| 10.1.20.248 | server5 | none                                                    |

##### 操作系统初始化

```bash
#K8S安装环境初始化，在所有server上都要执行

1.在server1到server5主机上修改各自的主机名（以server1为例）
...
| 主机    | IP          | 角色    |
| ------- | ----------- | ------- |
| server1 | 10.1.20.203 | primary |
| server2 | 10.1.20.251 | standby |

##### 安装部署postgresql11

```bash
分别在server1和server2上部署postgresql11
...
cat >>/etc/hosts << EOF
10.1.20.203 pg-test-1
10.1.20.251 pg-test-2
EOF
...
host    replication     replication     10.1.20.203/32   md5
host    replication     replication     10.1.20.251/32   md5
host    all             postgres        10.1.20.203/32   trust
host    all             postgres        10.1.20.251/32   trust
host    all             repl            10.1.20.203/32   trust
host    all             repl            10.1.20.251/32   trust
EOF
```
...
```bash
pgpool-II是一个集连接池、主备切换与负载均衡于一身的数据库连接件,需要在server1和server2上都安装，以下命令在两台机器都要执行
...
exit 0
EOF
chmod +x /usr/bin/ip_w 
chmod +x /usr/bin/arping_w

#给postgres用户增加sudo权限，让可执行ip和arping(两台机器都执行)
cat >>/etc/sudoers <<'EOF'
...
EOF
#配置pcp.conf，用于做集群密码管理
pg_md5 test
echo 'pgpool:098f6bcd4621d373cade4e832627b4f6' >>/etc/pgpool-II/pcp.conf	#字符串是pg_md5 test的结果
pg_md5 -f /etc/pgpool-II/pgpool.conf -m -u postgres test									#test是连接vip的密码
...
standby_mode = 'on'
primary_conninfo = 'user=replication password="test" host=10.1.20.203 port=5433 sslmode=prefer sslcompression=0 krbsrvname=postgres target_session_attrs=any'
EOF
2.在standby上执行
cat >/home/pgdata/pgsql/data/recovery.conf <<EOF
standby_mode = 'on'
primary_conninfo = 'user=replication password="test" host=10.1.20.251 port=5433 sslmode=prefer sslcompression=0 krbsrvname=postgres target_session_attrs=any'
EOF
...
```bash
iptables -A INPUT -m state --state NEW -m tcp -p tcp --dport 5432 -j ACCEPT -m comment --comment "pgpool" 
sudo -u postgres psql -h 10.1.20.199 -p 5432			<=输入密码：test
```
...
```bash
redis作为缓存服务器只需要在server1上安装即可，生产环境需要安装redis集群
...
mv /opt/redis-5.0.8/ /opt/redis && cd /opt/redis
make && make install
...
|     ip      | hostname |         角色          |
| :---------: | :------: | :-------------------: |
| 10.1.20.203 | server1  | gls001.longchuang.com |
| 10.1.20.251 | server2  | gls002.longchuang.com |
| 10.1.20.197 | server3  | gls003.longchuang.com |
| 10.1.20.195 | server4  | gls004.longchuang.com |
...
mkdir /home/glusterd
sed -i 's#var#lib/home#g' /etc/glusterfs/glusterd.vol
systemctl start glusterd.service
systemctl enable glusterd.service
cat >>/etc/hosts<<EOF
10.1.20.203 gls001.longchuang.com
10.1.20.251 gls002.longchuang.com
10.1.20.197 gls003.longchuang.com
10.1.20.195 gls004.longchuang.com
EOF
...
#检查节点连接状态
gluster peer status
gluster peer probe gls003.longchuang.com	#添加节点
gluster peer detach gls003.longchuang.com	#删除节点
```
...
#创建k8s卷
1.新建k8s卷并查看卷信息
gluster volume create k8s-volume replica 2 transport tcp gls001.longchuang.com:/home/gfs_data gls002.longchuang.com:/home/gfs_data gls003.longchuang.com:/home/gfs_data gls004.longchuang.com:/home/gfs_data
gluster volume info
...
http://10.1.20.203:8088/			用户名：admin				密码：Harbor12345
...
| :---------: | :------: | :-------: |
| 10.1.20.203 | server1  |     0     |
| 10.1.20.251 | server2  |     1     |
| 10.1.20.243 | server3  |     2     |
...
#为集群内3台机器创建各自的broker.id，server1的id是0，server2的是1,server3的是2，以server1为例，操作命令如下
cat >/home/kafka/zookeeper/data/myid << EOF
0
EOF

#在3台机器上都配置并启动zookeeper(注意：本机ip写成0.0.0.0，以下以server1为例)
...
server.1=10.1.20.203:2888:3888
server.2=10.1.20.197:2888:3888
/opt/kafka/bin/zookeeper-server-start.sh -daemon /opt/kafka/config/zookeeper.properties

#3台机器都配置并启动kafka（注意修改broker.id为各自的id，以下配置以server1为例）
vim /opt/kafka/config/server.properties
broker.id=0
listeners=PLAINTEXT://10.1.20.203:9092
...
zookeeper.connect=10.1.20.203:2181,10.1.20.251:2181,10.1.20.197:2181
...
#在任意机器上测试结果如下
[root@server2 ~] /opt/kafka/bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 2 --partitions 3 --topic test2
Created topic test2.
```
...
```bash
#在server1、2、3、4上分别安装kuberneres1.18.0
1.配置yum源
...
4.仅在server1上启动kubelet(在执行kubeadm init命令前kubelet正常都会无限重启)
...
```bash
#在主节点server1上用kubeadm初始化，再把server2和server3加入到集群，加入时node节点不要启动kubelet
...
记录信息如下：
kubeadm join 10.1.20.203:6443 --token ef9ei2.rketat03f75x60d8 \
...
kubeadm join 10.1.20.203:6443 --token hfwpy8.zrax1zk1z7v7dw0q  --discovery-token-ca-cert-hash sha256:eaa04fd29d76a3b4aa009e0f6d0e1c5727c0ea85c90f9be9c912d784e1109f42
```

##### 创建k8s-volume卷

```bash
gluster volume create k8s-volume replica 2 transport tcp \
gls001.longchuang.com:/home/gfs_data gls002.longchuang.com:/home/gfs_data gls003.longchuang.com:/home/gfs_data gls004.longchuang.com:/home/gfs_data
gluster volume info
...
```bash
#创建卷
在任意节点创建卷再启动卷即可，各节点上的目录会自动创建

gluster volume create k8s-volume-rabbit gls001.longchuang.com:/home/gfs_data_rabbit gls002.longchuang.com:/home/gfs_data_rabbit gls003.longchuang.com:/home/gfs_data_rabbit gls004.longchuang.com:/home/gfs_data_rabbit
gluster volume start k8s-volume-rabbit
...
![image-20201030134647864](https://tva1.sinaimg.cn/large/0081Kckwgy1gk7b8cxvc9j31400730t5.jpg)
...
```bash
docker会通过https从harbor上拉取镜像，所以需要在本地安装证书，而k8s调度资源是任意服务器，所以需要在server1-4都配置
...
#需要修改的文件
pom.xml
src/main/java/com/longchuang/software/LongChuangSoftwareApplication.java
src/main/resources/bootstrap.yml
src/main/java/com/longchuang/enterprise/filter/ParamsFilter.java
src/main/java/com/longchuang/enterprise/security/SecurityConfig.java

#第1个文件：bootstrap.yml
vim src/main/resources/bootstrap.yml

1.在bootstrap.yml文件中修改端口
server:
  port: 80
2.在bootstrap.yml文件中修改redis主机为本机
  redis:
    host: 10.1.20.203												#注：这是server1的ip，因为redis是安装在server1上的
    port: 6379
    password: test
    timeout: 500
3.在bootstrap.yml文件中删除eureka配置
eureka:
  instance:
    prefer-ip-address: true
  client:
    service-url:
      defaultZone: http://sslc:sslc@127.0.0.1:8761/eureka/
4.修改Spring Cloud Config配置
（微服务longchuang-edgecomputing-kafka不需要配置）
  cloud:
    config:
    ...
    kubernetes:
      reload:
        enabled: true
5.在bootstrap.yml文件中修改hystrix配置
hystrix:
  command:
    default:
      execution:
...
6.在bootstrap.yml文件中修改Actuator配置
（微服务longchuang-order，longchuang-mq-consumer不需要配置）
...
management:
  endpoint:
    ...
    info:
      enabled: true
7.在bootstrap.yml文件中修改RabbitMQ配置
（微服务longchuang-order，longchuang-mq-consumer需要配置）
    stream:
      binders:
        defaultRabbit:
          type: rabbit
          environment:
            spring:
              rabbitmq:
                ...
                password: test
                virtual-host: /
8.在bootstrap.yml文件中修改Mail配置	
（微服务longchuang-mail 需要配置）
  mail:
...
#修改第2个文件：LongChuangSoftwareApplication.java文件
vim src/main/java/com/longchuang/software/LongChuangSoftwareApplication.java

1.在LongChuangSoftwareApplication.java文件中删除eureka注解
(微服务longchuang-edgecomputing-kafka不用删除)

import org.springframework.cloud.netflix.eureka.EnableEurekaClient;
@EnableEurekaClient

2、在LongChuangSoftwareApplication.java文件中添加EnableScheduling注解
（微服务longchuang-edgecomputing-kafka不用添加）
...
# 修改第3个文件：pom.xml文件
vi pom.xml
1.在pom.xml文件中删除spring-cloud-starter-netflix-eureka-client依赖
（微服务longchuang-edgecomputing-kafka不需要删除依赖）
		<dependency>
			<groupId>org.springframework.cloud</groupId>
			<artifactId>spring-cloud-starter-netflix-eureka-client</artifactId>
		</dependency>
2、在pom.xml文件中添加spring-cloud-starter-kubernetes-netflix、spring-cloud-kubernetes-config和spring-cloud-starter-netflix-ribbon依赖
（微服务longchuang-edgecomputing-kafka不需要添加依赖）
		<dependency>
			<groupId>org.springframework.cloud</groupId>
...
3、在pom.xml文件中添加spring-boot-starter-actuator和
（微服务longchuang-order和longchuang-mq-consumer不需要添加依赖）
		<dependency>
			<groupId>org.springframework.boot</groupId>
			<artifactId>spring-boot-starter-actuator</artifactId>
		</dependency>

4.把longchuang-common-core改为sto-common-core

# api工程需要修改第4个文件：ParamsFilter.java文件
vi src/main/java/com/longchuang/enterprise/filter/ParamsFilter.java
pom.xml
1、在ParamsFilter.java文件中修改doFilter方法
（微服务longchuang-enterprise-api和longchuang-base-api需要修改）
    @Override
...
# api工程需要修改第5个文件：SecurityConfig.java文件
vi src/main/java/com/longchuang/enterprise/security/SecurityConfig.java

1.在SecurityConfig.java文件中configure方法中添加免登录URL
（微服务longchuang-enterprise-api和longchuang-base-api需要编辑）
...
let baseUrl = 'http://10.1.20.203:31002' // 其它接口
let loginUrl = 'http://10.1.20.203:31002' // 登录和登出
...
vim metro-enterprise-vue/src/company/SingleSpaConfig.js
        name: 'iot',
        entry: '//10.1.20.203:31006',
        name: 'onlineDevice',
        entry: '//10.1.20.203:8200',
...
```bash
vim /opt/maven/config/setting.html
<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0"
          xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
[root@server1 longchuang-account]#  docker pull reg.longchuang.com/shentong/longchuang-account:v1.0
...
```bash
[root@server1 from_gitlab]# kubectl get node
NAME      STATUS     ROLES    AGE     VERSION
server1   NotReady   master   6d20h   v1.18.0
server2   Ready      <none>   6d20h   v1.18.0
server3   Ready      <none>   6d20h   v1.18.0

排查：
kubectl describe node server1
Warning  ImageGCFailed  25m    kubelet, server1  failed to get imageFs info: unable to find data in memory cache
...
2.从192.168.50.121的mysql上同步axon库的表结构和数据到10.1.20.203的mysql

3.修改config工程和energy工程的连接数据库为10.1.20.203
sed -i 's#www.buduncle.top#10.1.20.203#g' /from_gitlab/metro-config/target/classes/config/longchuang-energy-test.yml
sed -i 's#www.buduncle.top#10.1.20.203#g' /from_gitlab/longchuang-energy/src/main/resources/bootstrap.yml
...
sed -i  ''  '/localhost/s#localhost:8100#10.1.20.203:31006#g' src/company/SingleSpaConfig.js
npm install
node build/build.js company
scp -r dist_company root@10.1.20.203:/tmp
...
3.打开浏览器
http://10.1.20.203:31005/emcs/bigsystem
